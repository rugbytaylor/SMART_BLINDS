...
#define NEUTRAL 1500
#define CLOSE_DIRECTION 0 // direction to close the blinds (1 is open)
...
int write_txt_closed(const char *filename) {
...
int main() {
    ...
    if (blinds_state == 2) { return 0;} //if already closed, done!
    else {
	//CREATE LOCK FILE
	...
	printf("Starting blinds_time_close...\n");
	int pi;
	if (!setup_pi(&pi)) { return 1;}
	spin_servo_direction(&pi, CLOSE_DIRECTION);

	int written = write_txt_closed(STATE_TXT);
